...
originalDf.drop_duplicates(subset =['name','address'], keep = 'first', inplace = True)
...
df['location'] = df['location'].apply(replaceNaNByString)
df['location'] = df['location'].apply(splitToList)
...
df['rate'] = df['rate'].apply(splitToList)
...
##
df.drop(df[df['approx_cost(for two people)'] > 1100.0].index, inplace = True)
...
for train_index, test_index in kf.split(train_x):
    ...
    randomForestRegression = RandomForestRegressor(max_depth=30, random_state=0, n_estimators=100)
    randomForestRegression.fit(X_train, y_train)
    y_pred = randomForestRegression.predict(X_test)
    rmse = np.sqrt(mean_squared_error(y_test, y_pred))
    if len(root_mean_square_errors) == 0:
        bestModel = randomForestRegression
        print(50*'$')
    elif rmse < min(root_mean_square_errors):
        bestModel = randomForestRegression
        print(50*'%')
...
# Plot the RMSEs for themodels trained using Cross-Validation folds:
plt.scatter(range(1, len(root_mean_square_errors) +1), root_mean_square_errors)
plt.gca().set_xticks(range(1, len(root_mean_square_errors) +1))
plt.gca().set_ylim([0,max(root_mean_square_errors) + 20])
plt.title('RMSE of different models', fontsize = 18)
...
####################################### 3.f ##################################################
# We are taking the best model created during the Cross-Validation process on
# the training data set in the question 3.e:

pred_y = randomForestRegression.predict(test_x)
rmse = np.sqrt(mean_squared_error(test_y, pred_y))

# add the RMSE calculated for the bestModel on the testing set to the RMSEs calculated
# during Cross-Validation on the training set:

root_mean_square_errors.append(rmse)
plt.scatter(range(1, len(root_mean_square_errors) +1), root_mean_square_errors)
plt.gca().set_xticks(range(1, len(root_mean_square_errors) +1))
plt.gca().set_ylim([0,max(root_mean_square_errors) + 20])
plt.title('RMSE of Cross-Validation models on the training set + bestModel on Testing set', fontsize = 14)
plt.xlabel('Model Number', fontsize = 12)
plt.ylabel('RMSE value', fontsize = 12)
plt.show()
